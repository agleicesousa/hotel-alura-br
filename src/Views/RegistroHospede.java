...
public class RegistroHospede extends JFrame {
    ...
    public void salvarHospede() {
        try {
            if (validarCampos()) {
                Date dateNascimentoUtil = txtDataN.getDate();
                Instant instant = Instant.ofEpochMilli(dateNascimentoUtil.getTime());
                ...
                Hospedes hospedes = new Hospedes(txtNome.getText(), txtSobrenome.getText(), 
                        LocalDate.parse(dataNascimentoFormatada, formatter),
                        txtNacionalidade.getSelectedItem().toString(), txtTelefone.getText(), nreserva);
                
                this.hospedesControl.guardarH(hospedes);
            ...
            JOptionPane.showMessageDialog(this, "O número da reserva deve ser um valor válido.");
        }
    }
